...
def treinar_lstm(pasta_dados):
    """Treina e avalia LSTM.
    
    Args:
        pasta_dados: caminho da pasta com ficheiros lstm_treino.npz e lstm_teste.npz
    """
    from tensorflow.keras.callbacks import EarlyStopping
    
    print("\n  ── LSTM ──")
    
    # Carregar dados dos ficheiros .npz
    treino_npz = np.load(os.path.join(pasta_dados, 'lstm_treino.npz'))
    teste_npz = np.load(os.path.join(pasta_dados, 'lstm_teste.npz'))
