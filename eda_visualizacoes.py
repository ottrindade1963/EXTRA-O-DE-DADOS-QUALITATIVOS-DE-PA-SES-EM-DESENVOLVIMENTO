...
import numpy as np
import os
from eda_config import COLUNAS_NUMERICAS, NOMES_CURTOS, OUTPUT_DIR
...
def plot_missing(df):
    """Mapa de calor dos valores ausentes por variável e década."""
...
def plot_histogramas(df):
    """Histogramas com KDE para cada variável numérica."""
    print("📊 2. Histogramas...")
...
# ── 5. Evolução temporal (médias globais) ────────────────────────
def plot_evolucao_temporal(df):
    """Linhas de evolução temporal das médias por ano."""
...
# ── 6. Top 10 países por PIB per capita (último ano) ─────────────
def plot_top_paises(df):
    """Barras horizontais dos top 10 países por PIB per capita."""
    print("🏆 6. Top 10 países...")
    ultimo_ano = df["ano"].max()
    top = (df[df["ano"] == ultimo_ano]
           .nlargest(10, "pib_per_capita_ppc")
           [["pais", "pib_per_capita_ppc"]])
...
# ── 7. Scatter: PIB vs Industrialização ──────────────────────────
def plot_scatter_pib_industria(df):
    """Dispersão PIB per capita vs Valor agregado industrial."""
    print("🔵 7. Scatter PIB vs Indústria...")
    ultimo_ano = df["ano"].max()
    d = df[df["ano"] == ultimo_ano].dropna(subset=["pib_per_capita_ppc", "valor_agregado_industrial_percent_pib"])

    fig, ax = plt.subplots(figsize=(10, 7))
    sns.scatterplot(data=d, x="pib_per_capita_ppc", y="valor_agregado_industrial_percent_pib",
                    size="populacao_total", sizes=(20, 500), alpha=0.6, ax=ax, legend=False)
...
# ── 8. Violin plot: Emprego industrial por década ────────────────
def plot_violin_emprego(df):
    """Violin plot do emprego industrial por década."""
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.violinplot(data=df2, x="decada", y="emprego_industria_percent_emprego_total",
                   palette="muted", ax=ax)
...
def plot_pairplot(df):
    """Pairplot de 4 variáveis-chave (amostra de 500 registros)."""
    print("🔷 9. Pairplot...")
    cols = ["pib_per_capita_ppc", "comercio_percent_pib",
            "valor_agregado_industrial_percent_pib", "emprego_industria_percent_emprego_total"]
