...
def treinar_dataset(chave):
    ...
    try:
        _, met_rf, y_pred_rf, y_real_rf, _ = treinar_random_forest(
            f"{chave}_trees_treino.csv",
            f"{chave}_trees_teste.csv"
        )
    ...
    # 3. LSTM
    try:
        _, met_lstm, y_pred_lstm, y_real_lstm, _ = treinar_lstm(chave)
    ...
    try:
        _, met_sar, y_pred_sar, y_real_sar, _ = treinar_sarimax(
            f"{chave}_sarimax_treino.csv",
            f"{chave}_sarimax_teste.csv",
            f"{chave}_sarimax_exogenas.csv"
        )
    ...
    try:
        _, met_tft, y_pred_tft, y_real_tft, _ = treinar_tft(
            f"{chave}_tft_treino.csv",
            f"{chave}_tft_teste.csv"
        )
...
def executar_treino_avaliacao():
    ...
    for chave in DATASETS:
        # Verificar se pelo menos um ficheiro do dataset existe na raiz
        if not os.path.exists(f"{chave}_trees_treino.csv"):
            print(f"\n  ⚠️ Ficheiros do dataset '{chave}' não encontrados na raiz.")
            continue
            
        resultados, tabela = treinar_dataset(chave)
